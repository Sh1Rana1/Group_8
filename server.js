...
async function api(req,res,url) {
  ...
  if (req.method==='POST'&&url.pathname==='/api/admin/recharge') {
    const user=auth(req,res,'admin'); if(!user)return; const body=await readBody(req),db=readDb();
    const room=db.rooms[body.roomKey]; const amount=Number(body.amount)||0;
    if(!room) return json(res,404,{error:'宿舍不存在'});
    if(amount<=0) return json(res,400,{error:'充值金额无效'});
    room.balance=+(room.balance+amount).toFixed(2);
    if(room.powerState==='arrears'&&room.balance>0) room.powerState='normal';
    room.studentIds.forEach(id=>{
      const data=db.studentData[id]; if(!data)return;
      data.room.balance=room.balance; data.room.powerState=room.powerState;
      data.electricity.recharges=(data.electricity.recharges||[]);
      data.electricity.recharges.unshift({id:'RC'+Date.now(),amount,channel:'管理员台代充',time:'刚刚'});
      data.notifications.unshift({id:'N'+Date.now(),type:'power',level:'success',title:'管理员代充成功',text:`管理员已为宿舍充值 ¥${amount.toFixed(2)}，当前余额 ¥${room.balance.toFixed(2)}。`,time:'刚刚',read:false});
    });
    writeDb(db); return json(res,200,{ok:true,balance:room.balance,powerState:room.powerState});
  }

...
async function api(req,res,url) {
  ...
  if (req.method==='PUT'&&url.pathname==='/api/my-data') {const user=auth(req,res,'student');if(!user)return;const body=await readBody(req),db=readDb();if(!body.data||typeof body.data!=='object')return json(res,400,{error:'数据格式错误'});const incoming=body.data,room=db.rooms[user.roomKey];if(room){const cut=room.powerState==='manual'||room.powerState==='violation';const num=(v,fb)=>{const n=Number(v);return Number.isFinite(n)?n:fb};if(cut){incoming.room={...(incoming.room||{}),balance:room.balance,powerState:room.powerState,power:room.power,todayKwh:room.todayKwh,monthKwh:room.monthKwh};incoming.electricity={daily:room.daily||[],recharges:room.recharges||[]};}else{let nextState=incoming.room?.powerState||room.powerState||'normal';if(nextState!=='normal'&&nextState!=='arrears') nextState=room.powerState||'normal';if(room.powerState==='arrears'&&nextState==='normal'){const nb=num(incoming.room?.balance,room.balance||0);if(!(nb>0)) nextState='arrears';}if(nextState==='normal'&&room.powerState==='arrears'){/* allowed via recharge */}room.balance=Math.max(0,num(incoming.room?.balance,room.balance||0));room.powerState=nextState;room.power=nextState==='normal'?num(incoming.room?.power,room.power||0):0;room.todayKwh=Math.max(0,num(incoming.room?.todayKwh,room.todayKwh||0));room.monthKwh=Math.max(0,num(incoming.room?.monthKwh,room.monthKwh||0));room.daily=Array.isArray(incoming.electricity?.daily)?incoming.electricity.daily:(room.daily||[]);room.recharges=Array.isArray(incoming.electricity?.recharges)?incoming.electricity.recharges.slice(0,50):(room.recharges||[]);}room.studentIds.forEach(id=>{const existing=db.studentData[id];if(!existing)return;existing.room={...(cut?existing.room:incoming.room),students:room.studentIds.map(uid=>db.users.find(u=>u.id===uid)?.name).filter(Boolean),balance:room.balance,powerState:room.powerState,power:room.power,todayKwh:room.todayKwh,monthKwh:room.monthKwh};existing.electricity={daily:room.daily||[],recharges:room.recharges||[]};existing.updatedAt=incoming.updatedAt||Date.now()})}db.studentData[user.id]={...incoming,room:db.rooms[user.roomKey]?{...(incoming.room||{}),balance:db.rooms[user.roomKey].balance,powerState:db.rooms[user.roomKey].powerState,power:db.rooms[user.roomKey].power,todayKwh:db.rooms[user.roomKey].todayKwh,monthKwh:db.rooms[user.roomKey].monthKwh}:incoming.room,water:incoming.water,notifications:incoming.notifications};writeDb(db);return json(res,200,{ok:true,updatedAt:Date.now()});}
  ...
  if (req.method==='POST'&&url.pathname==='/api/admin/recharge') {
    const user=auth(req,res,'admin'); if(!user)return; const body=await readBody(req),db=readDb();
    const amount=Number(body.amount); if(!Number.isFinite(amount)||amount<=0||amount>10000) return json(res,400,{error:'请输入有效金额'});
    const room=db.rooms[body.roomKey]; if(!room)return json(res,404,{error:'宿舍不存在'});
    room.balance=+((room.balance||0)+amount).toFixed(2);
